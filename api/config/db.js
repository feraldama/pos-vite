...
const pool = new Pool({
  ...
  database: process.env.DB_NAME || "alquilerprendas",
  max: 15,
...
const TABLE_NAMES = new Set([
  ...
  "vendedor", "venta", "ventacredito", "ventacreditopago", "ventaproducto",
]);
...
function translateQuery(sql) {
  ...
  for (let i = 0; i < parts.length; i += 2) {
    ...
    seg = seg.replace(/(^|[^\w"])([A-Za-z_][A-Za-z0-9_]*)(?![\w"(])/g, (full, pre, word, off, str) => {
...
function shapeResult(text, res) {
  ...
  if (cmd === "SELECT" || (res.rows && res.rows.length && cmd !== "INSERT" && cmd !== "UPDATE" && cmd !== "DELETE")) {
    return res.rows;
  }
  const shaped = {
